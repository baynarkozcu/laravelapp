        ...
        <div class="col-10">
            ...
            <form action="{{ route('stockAdd') }}" method="post">
                ...
                <select class="form-control" name="product_id">
                    ...
                    @foreach($products as $product)
                        <option value="{{$product->UUID}}">{{$product->Ad}}</option>
                    @endforeach
                </select>

                <hr>
                <input type="number" name="stokAdedi" placeholder="Stok Adeti Giriniz" id="" class="form-control">
                <hr>
            ...
            <table id="example" class="display" style="width:100%">
                ...
                <tr>
                    ...
                    <td>{{ $val->stokAdedi }}</td>
                    @php
                        $product= \App\Models\Product::where('UUID', $val->product_id)->first();
                    @endphp
                    <td>{{ $product->ad }}</td>
                    <td>{{ $val->created_at }}</td>
                    <td>
                        <a href="{{ route('stockDelete')."/".$val->UUID }}" class="btn btn-danger"> SİL </a>
                        <a href="{{ route('stockUpdate')."/".$val->UUID }}"
